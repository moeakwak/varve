...
def test_invalidation_reason_priority() -> None:
    assert invalidation_reason(_components(source={"a": "1"}), _components(source={"a": "2"})) == "source changed"
    assert invalidation_reason(_components(config={"x": 1}), _components(config={"x": 2})) == "config: x 1 -> 2"
    assert invalidation_reason(_components(values={"v": 1}), _components(values={"v": 2})) == "value: v 1 -> 2"
...
def test_decide_single_rows() -> None:
    ...
    assert decide_single(current_key="sha256:a", current_components=_components(), success=_single(), attempt=marker, produces_exist=True).status == "dirty"
    assert decide_single(current_key="sha256:a", current_components=_components(), success=_single(), attempt=None, produces_exist=True).status == "hit"
    assert decide_single(current_key="sha256:a", current_components=_components(), success=_single(), attempt=None, produces_exist=False).status == "artifact-missing"
    assert decide_single(current_key="sha256:b", current_components=_components(source={"x": "y"}), success=_single(), attempt=None, produces_exist=True).status == "stale"
    assert decide_single(current_key="sha256:a", current_components=_components(), success=None, attempt=None, produces_exist=False).status == "no-cache"
