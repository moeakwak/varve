...
`status.py` converts probes into immutable cell-aware view models, groups concrete cells by base stage, and compares current source components with the previous successful record for stale stages. Every concrete stage is still probed before display selection and folding, so folding cannot change cache decisions or exact cell identity. Aggregate status uses the centralized least-to-most-severe order in `engine.state`; aggregate duration sums recorded cell durations and carries the recorded count so the renderer can expose missing values. `cli/status.py` renders the folded summary, axis-column cell tables, single-stage key inputs, progressively expanded dependency DAG, and source change markers; it consumes base names, canonical coordinates, axis order, and logical needs from the view model instead of parsing concrete stage names. Qualified names, stored source locators, and known edge reasons are compacted only at this rendering boundary. The command is read-only: it does not execute stages, initialize the store, or persist dependency graphs. A displayed decision key is the current read-only cache decision input, not a promise that the next execution will commit the same final key after recording config access.
...
Generated `status` output folds concrete cells by base template by default. `--expand` dispatches by selection: a matrix base or an untargeted matrix pipeline renders cells with one column per declared axis, while an ordinary stage, concrete cell, or non-matrix pipeline retains the existing one-level source dependency view. `--all` likewise retains the complete source dependency tree for ordinary stages, concrete cells, and non-matrix pipelines; a matrix base or untargeted matrix pipeline must select one concrete cell first. `--deps` and `--deps-all` provide explicit source-dependency equivalents for exactly one concrete cell or ordinary stage. All four expansion flags remain mutually exclusive.

Managed matrix artifacts are contained under `ctx.cell_out`, which is `<output-root>/.matrix/<base-stage>/<axis-name>=<canonical-id>/...`; each coordinate occupies one directory level in the stage's axis declaration order. For example, `score@bench=unimer,model=qwen3-vl-8b` writes under `.matrix/score/bench=unimer/model=qwen3-vl-8b/` while its concrete stage and store identity remains `score@bench=unimer,model=qwen3-vl-8b`. Relative `produces` declarations and batch yields resolve under `ctx.cell_out`; absolute paths must remain inside it. Store records continue to hold paths relative to the branch output root, so artifact existence checks, upstream reads, and stage clean operate from recorded paths rather than reconstructing physical paths. Ordinary stages retain `ctx.cell_out == ctx.out`.
